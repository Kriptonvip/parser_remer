...
import { futbolki, btFutbolki } from './data.js';
...
async function parseAndSave(url) {
  ...
  $('ul.product-options li').each((index, element) => {
    ...
    if(futbolki[articul] === undefined && btFutbolki[articul] === undefined) {
      return;
    };
    products.push({
      Name:`${productName}`,
      articulTTsport: articul,
      'offer id': futbolki[articul] || btFutbolki[articul],
      price: price.split(' ')[0],
...
async function main() {
  const urls = await readUrlsFromFile('futbolki.txt');
  const allProducts = [];
  let count = 0;
  ...
  const csvFilename = `products_combined_Futbolki.csv`;
