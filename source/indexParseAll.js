...
async function main() {
  ...
  const csvFilename = `products_combined_All.csv`;
  const csvContent = `Name;articulTTsport;offer id;price;Наличие;datajs;Модификация;Склад;Ссылка\n${allProducts.map(p => Object.values(p).join(';')).join('\n')}`;
