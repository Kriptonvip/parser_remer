...
async function downloadImages(imageUrls) {
  for (let i = 0; i < imageUrls.length; i++) {
    ...
    const imageFileName = path.join(downloadDirectory, `${lastPart}.jpg`);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log(`Изображение ${i + 1} успешно скачано: ${imageFileName}`);
      } else {
        console.error(`Ошибка при скачивании изображения ${i + 1}: Неверный статус ответа`);
      }
    } catch (error) {
      console.error(`Ошибка при скачивании изображения ${i + 1}: ${error.message}`);
    }
  }
}
...
async function parseAndSave(url, prodUrls) {
...
// Вывести URL изображения в консоль
  console.log("URL изображения:", imgVariant);
  const productName = $('h1').text();
  const artikulName = $('.js-replace-article').text().trim();
  console.log('artikulName', artikulName)
...
const priceText = $('.price__new-val').text();
...
if(pdfLink !== undefined) {
  ...
  const urlpdfff = 'https://remer.shop' + pdfLink

downloadAndConvertPDFToImages(urlpdfff, downloadDirectory);
}
...
async function main() {
  const urls = await readUrlsFromFile('urls_test.txt');
  const allProducts = [];
  const prodUrls = new Set();
  let count = 0;
  for (const url of urls) {
    const products = await parseAndSave(url, prodUrls);
    count++;
    console.log(`Product ${count} parse done!`)
    allProducts.push(...products);
    if(products[0][ "Тип строки"] === 'product'){
      const productsRepeat = await parseAndSave(url, prodUrls);
