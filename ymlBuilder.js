...
// Чтение CSV-файла
const csvData = await fs.readFile('products_combined.csv', 'utf-8');

// Преобразование CSV в объекты
const records = [];
await new Promise((resolve, reject) => {
  fsSync.createReadStream('products_combined.csv')
...
records.forEach(record => {
  ...
  const correctPrice = (ttSportArt) => {
	  if (isButRubber) return record['price'] * btObj[record['articulTTsport']][1];
  	  if (isButBlade) return (record['price'] * 0.8 + 290) / 0.725;
  	  if (isBlade) return (record['price'] * 0.75 + 290) / 0.725;
	  if (isDirectPrice) {
...
// Запись в XML-файл
await fs.writeFile('outputAll.xml', xmlString, 'utf-8');
