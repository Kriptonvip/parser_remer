...
async function parseAndSave(url) {
  ...
  $('ul.product-options li').each((index, element) => {
    ...
    const modification = (label.text().trim() === 'В наличии' || label.text().trim() === 'Нет в наличии') ? '' : label.text().trim();
    if(dataBlades[articul]) {
      return;
    };
    const isBut = brandName ===  'Butterfly' ? true : false; 
    const ozonArtikul = modification === '' ?  rus_to_latin(productName) : `${rus_to_latin(productName)}_${rus_to_latin(modification)}`;
    
    products.push({
      '№': '',
      'Артикул*': ozonArtikul.replace('raketka', 'rtk'),
      "Название товара": productName,
      "Цена, руб.*": isBut ?  Math.floor((price * 0.8 + 290)/0.725) : Math.floor((price * 0.75 + 290)/0.725),
      "Цена до скидки, руб.":  isBut ?  Math.floor((price * 0.8 + 290)/0.725) : Math.floor((price * 0.75 + 290)/0.725),
      "НДС, %*": "Не облагается",
...
async function main() {
  const urls = await readUrlsFromFile('rackets.txt');
  const allProducts = [];
  let count = 0;
  ...
  const csvFilename = `products_combined_RacketsOzon.csv`;
